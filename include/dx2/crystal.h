...
void Crystal::init_from_abc(Vector3d a, Vector3d b, Vector3d c){
    ...
    Matrix3d B_{{B.a[0][0], B.a[0][1], B.a[0][2]}, {B.a[1][0], B.a[1][1], B.a[1][2]}, {B.a[2][0], B.a[2][1], B.a[2][2]}};
    U_ = A_ * B_;
}

Crystal::Crystal(Vector3d a, Vector3d b, Vector3d c, gemmi::SpaceGroup space_group): space_group_(space_group) {
    init_from_abc(a,b,c);
}

Crystal::Crystal(json crystal_data){
    std::vector<std::string> required_keys = {
        "real_space_a", "real_space_b", "real_space_c", "space_group_hall_symbol"
    };
    for (const auto &key : required_keys) {
        if (crystal_data.find(key) == crystal_data.end()) {
            throw std::invalid_argument(
                "Key " + key + " is missing from the input crystal JSON");
        }
    }
    Vector3d rsa{{crystal_data["real_space_a"][0], crystal_data["real_space_a"][1],crystal_data["real_space_a"][2]}};
    Vector3d rsb{{crystal_data["real_space_b"][0], crystal_data["real_space_b"][1],crystal_data["real_space_b"][2]}};
    Vector3d rsc{{crystal_data["real_space_c"][0], crystal_data["real_space_c"][1],crystal_data["real_space_c"][2]}};
    space_group_ = *gemmi::find_spacegroup_by_name(crystal_data["space_group_hall_symbol"]);
...
gemmi::SpaceGroup Crystal::get_space_group() const {
    return space_group_;
}
...
Matrix3d Crystal::get_B_matrix() const {
    return B_;
}

Matrix3d Crystal::get_U_matrix() const {
    return U_;
}
