...
TEST(ExampleTests, CrystalTest) {
  ...
  std::array<double, 6> expected_cell = {10.246950765959598, 20.42057785666214, 5.744562646538029, 58.09405586165891, 57.06933979197713, 79.53690718052404};
  Matrix3d expected_A{
  ...
  std::array<double, 6> params = {cell.a, cell.b, cell.c, cell.alpha, cell.beta, cell.gamma};
  for (int i=0;i<6;i++){
    EXPECT_NEAR(params[i], expected_cell[i], 1E-12);
  }

  // Now do niggli reduction, and check the quantities against dials/dxtbx equivalent
  crystal.niggli_reduce();

  gemmi::UnitCell reduced_cell = crystal.get_unit_cell();
  std::array<double, 6> expected_reduced_cell = {5.744562646538028, 8.602325267042627, 17.34935157289747, 98.47679564504885, 92.30016540862056, 91.15952321378222};
  std::array<double, 6> reduced_params = {reduced_cell.a, reduced_cell.b, reduced_cell.c, reduced_cell.alpha, reduced_cell.beta, reduced_cell.gamma};
  for (int i=0;i<6;i++){
