...
TEST(ExampleTests, ReadArrayTest) {
    ...
    std::vector<double> xyzobs_px = read_array_from_h5_file<double>(fpath, array_name);
    // check a random value
    double expected_value = 528.86470588235295;
    EXPECT_EQ(xyzobs_px[10], expected_value);

    std::vector<std::size_t> flags_array = read_array_from_h5_file<std::size_t>(fpath, flags);
    // check a random value
