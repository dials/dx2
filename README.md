...
### Install rules when embedding

When dx2 is consumed via `add_subdirectory` or `FetchContent`, its install
rules are skipped by default so they don't pollute the parent project's
install tree. If you do want the parent's `install` step to also install
dx2's headers, library, and CMake package files, opt in with:

```
cmake -DDX2_INSTALL=ON ..
```

When dx2 is the top-level project, install rules are generated by default.
