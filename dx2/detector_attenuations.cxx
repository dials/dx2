...
// Just implement the required corrections for Si and CdTe detectors for now using NIST data.
...
double mu_rho_at_ev(double energy_ev, const std::array<double, N>& energy_data, const std::array<double, N>& mu_rho_data) {
...
double calculate_CdTe_mu_from_wavelength(double wavelength) {
    ...
    return mu_rho_at_ev(ev, CdTe_energy_data, CdTe_mu_rho_data) * CdTe_density / 10.0;
}

double calculate_silicon_mu_from_wavelength(double wavelength) {
    double ev = factor_ev_angstrom / wavelength;
    return mu_rho_at_ev(ev, silicon_energy_data, silicon_mu_rho_data) * silicon_density / 10.0;
}

// Calculate mu (the material absorption coefficient) in mm^-1 at the wavelength (in angstrom).
double calculate_mu_for_material_at_wavelength(const std::string& material, double wavelength) {
    if (material == "Si") {
        ...
        throw std::invalid_argument("Only Silicon or CdTe detector absorption coefficients currently implemented");
    }
}
